...
def analyze_class_distribution(labels_dir):
    ...
    # Durchsuche alle .txt-Dateien rekursiv im labels_dir
    for label_file in Path(labels_dir).rglob("*.txt"):
        with open(label_file, 'r') as f:
            for line in f:
                parts = line.strip().split()
                if parts:
                    # Annahme: Erste Zahl ist die Klassen-ID
...
def main():
    ...
    labels_dir = os.path.join(base_path, "labels/train")  # Spezifiziere den train-Ordner
